...
async def ack_then_followup(interaction: discord.Interaction, *, content=None, embed=None, view=None):
    """Always acknowledge the interaction, then send a fresh ephemeral message with the next step."""
    try:
        if not interaction.response.is_done():
            try:
                ...
                try:
                    ...
                    pass
        await interaction.followup.send(content or "Continue", embed=embed, view=view, ephemeral=True)
    except Exception:
        try:
...
async def smart_edit(interaction: discord.Interaction, *, content=None, embed=None, view=None):
    """Choose the safest edit path for this interaction without double-responding.
    Priority:
      1) interaction.response.edit_message(...)
      2) interaction.edit_original_response(...)
      3) interaction.message.edit(...)
      4) interaction.followup.send(..., ephemeral=True)
    """
    try:
        if not interaction.response.is_done():
            await smart_edit(interaction, content=content, embed=embed, view=view)
            return
        try:
            await smart_edit(interaction, content=content, embed=embed, view=view)
            return
        ...
        try:
            await interaction.followup.send("Update failed. Please try again.", ephemeral=True)
        except Exception:
            pass
...
async def safe_update(interaction: discord.Interaction, *, content=None, embed=None, view=None):
    """Robust updater for component and modal interactions.
    - Tries to defer_update() (component) or defer(ephemeral=True) (modal/unknown).
    - Then edits interaction.message if available.
    - Falls back to followup.send(ephemeral=True) if edit not possible.
    """
    try:
        if not interaction.response.is_done():
            try:
        ...
        if getattr(interaction, "message", None) is not None:
            try:
                ...
                pass
        # Fallback: send a fresh ephemeral
        await interaction.followup.send(content or "Updated.", embed=embed, view=view, ephemeral=True)
    except Exception as e:
        try:
            await interaction.followup.send("Update failed. Please try again.", ephemeral=True)
...
class SummaryView(discord.ui.View):
    @discord.ui.button(label="Reminder: Off", style=discord.ButtonStyle.secondary, custom_id="tm:rem_no")
    async def rem_off(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.req.want_reminder = False
        await self.send_or_update(interaction)

    @discord.ui.button(label="Reminder: On", style=discord.ButtonStyle.success, custom_id="tm:rem_yes")
    async def rem_on(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.req.want_reminder = True
        await self.send_or_update(interaction)
    ...
    async def send_or_update(self, interaction: discord.Interaction):
        ...
        # Rebuild a fresh SummaryView so components are bound to the new message
        new_view = SummaryView(self.cog, self.req.user_id, self.req.discipline, self.req.training, self.req.days, self.req.fee_per_day, self.req.reference)
        new_view.req.want_reminder = self.req.want_reminder
        # tweak button styles
        for child in new_view.children:
            if isinstance(child, discord.ui.Button):
                if child.custom_id == "tm:rem_yes":
                    child.style = discord.ButtonStyle.success if new_view.req.want_reminder else discord.ButtonStyle.secondary
                if child.custom_id == "tm:rem_no":
                    child.style = discord.ButtonStyle.success if not new_view.req.want_reminder else discord.ButtonStyle.secondary
        await ack_then_followup(interaction, content="Review and submit to Admin.", embed=embed, view=new_view)
...
class SubmitButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await smart_edit(interaction, content="Sent to Admin. You'll be notified on any change.", embed=None, view=None)
...
class TrainingManager(commands.Cog):
    ...
    async def cog_app_command_error(self, interaction: discord.Interaction, error):
        log.exception("App command error: %r", error)
        try:
