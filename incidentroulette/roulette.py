"""
DEBUG VERSION - Use this to find the row issue
"""
...
class RouletteView(View):
    ...
    def _build_for_current(self):
        ...
        print(f"\nDEBUG: Building UI for call {idx}")
        print(f"DEBUG: Total ROLES: {len(ROLES)}")
        ...
        for i, role in enumerate(ROLES):
            ...
            print(f"DEBUG: Adding {role} select to row {sel.row}")
            self.add_item(sel)
        ...
        confirm.row = 3
        print(f"DEBUG: Adding Confirm button to row 3")
        ...
        cancel.row = 4
        print(f"DEBUG: Adding Cancel button to row 4")
        self.add_item(cancel)
        
        print(f"DEBUG: Total items added: {len(self.children)}")
        print(f"DEBUG: Items per row:")
        for row in range(5):
            items_in_row = [c for c in self.children if getattr(c, 'row', 0) == row]
            print(f"  Row {row}: {len(items_in_row)} items")
