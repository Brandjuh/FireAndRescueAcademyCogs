# cookie_manager.py (v5.3 - with session reuse)
...
class CookieManager(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # Session reuse
        self._shared_session: Optional[ClientSession] = None
    ...
    async def _save_cookies(self, cookies: List[Dict[str, Any]], meta: Dict[str, Any]):
        ...
        # Mark session as invalidated so it gets recreated with new cookies
    ...
    async def get_session(self) -> ClientSession:
        """Get shared session with current cookies. Session is reused across calls."""
        async with self._cookie_lock:
            # If session doesn't exist, is closed, or cookies were updated, create new one
            if (self._shared_session is None or 
                self._shared_session.closed or 
                self._session_invalidated):
                
                # Close old session if it exists and isn't closed
                if self._shared_session and not self._shared_session.closed:
                    try:
                        await self._shared_session.close()
                    except Exception:
                ...
                # Create new session with current cookies
                self._shared_session = await self.build_session_from_store()
                self._session_invalidated = False
                log.debug("Created new shared session")
    ...
    async def status(self, ctx: commands.Context):
        ...
        lines.append(f"Shared session: {'active' if self._shared_session and not self._shared_session.closed else 'inactive'}")
