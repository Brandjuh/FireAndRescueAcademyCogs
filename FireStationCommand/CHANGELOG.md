...
- Added repeatable parking lot upgrades for extra vehicle capacity with increasing configurable costs.
- Regenerated all mission, vehicle, and equipment catalog images in the sharper clean 2D cartoon game-asset style.
- Added level-up unlock summaries that list newly available station upgrades, features, missions, vehicles, equipment, trainings, and extensions.
...
- Turnout takeover now keeps the takeover offer active when an ineligible member clicks it, returns clear feedback to the original player, and only closes the takeover view after a successful transfer.
