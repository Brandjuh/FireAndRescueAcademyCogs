...
class GitHubSync:
    ...
    def parse_typescript_export(self, content: str) -> Optional[Dict[int, Any]]:
        ...
        try:
            ...
            match = re.search(r'export\s+default\s+({[\s\S]+?})\s*(?:satisfies|as\s+const)?.*?;?\s*
...
, content)
            ...
            # Handle unquoted keys - match word followed by colon
            # This handles both simple keys and nested object keys
            ...
            if hasattr(e, 'pos') and e.pos:
                ...
                end = min(len(obj_str), e.pos + 100)
                context = obj_str[start:end]
                log.error(f"Context around error: ...{context}...")
            return None
        except Exception as e:
