...
class TextCAD(commands.Cog):
    def __init__(self, bot, config: Config, econ: EconomyBridge):
        ...
        self.guild.register(**GUILD_DEFAULT, incidents={})
        self.member.register(**MEMBER_DEFAULT)
    ...
    def _mk_unit_id(self, ut: UnitType) -> str:
        prefix = {
            "engine":"ENG","ladder":"LAD","rescue":"RES","ems":"EMS",
            "patrol":"LEO","arff":"ARF","tow":"TOW","command":"CMD"
        }[ut.value]
    ...
    def _deserialize_inc(self, data: dict) -> Incident:
        ...
        inc = Incident(
            iid=data["iid"], name=data["name"], tier=int(data["tier"]), requirements=req,
            created_ts=data["created_ts"], resolved_ts=data.get("resolved_ts"),
            channel_id=data.get("channel_id"), thread_id=data.get("thread_id")
        )
        for key, pd in (data.get("participants") or {}).items():
            inc.participants[key] = Participant(
                user_id=int(pd["user_id"]), unit_id=pd["unit_id"],
                unit_type=UnitType(pd["unit_type"]), status=Status(pd["status"]),
                joined_ts=pd.get("joined_ts",0), arrival_ts=pd.get("arrival_ts"),
                transport_ts=pd.get("transport_ts"), clear_ts=pd.get("clear_ts"),
            )
        return inc
    ...
    async def _persist_inc(self, guild: discord.Guild, inc: Incident):
        ...
        gi[inc.iid] = {
            ...
            "participants": {
                key: {
                    "user_id": p.user_id, "unit_id": p.unit_id, "unit_type": p.unit_type.value,
                    "status": p.status.value, "joined_ts": p.joined_ts, "arrival_ts": p.arrival_ts,
                    "transport_ts": p.transport_ts, "clear_ts": p.clear_ts
                }
        ...
        try:
            ...
            while True:
                ...
                if mode == "shift" and g.get("shift_end_ts",0) <= now_ts():
                    await self.guild(guild).shift_active.set(False); break
    ...
    async def _select_unit_for_action(self, ctx: commands.Context) -> Optional[Tuple[str, dict]]:
        mid = self.member(ctx.author); units = await mid.units()
        if not units:
    ...
    async def shiftstart(self, ctx: commands.Context, duration_min: int = 60):
        if (await self.guild(ctx.guild).mode()) != "shift":
            await ctx.reply("Mode is not 'shift'.", ephemeral=True); return
    ...
    async def on(self, ctx: commands.Context, unit: str):
        try:
            ...
            await ctx.reply("Unknown unit type.", ephemeral=True); return
        fee = await self.guild(ctx.guild).duty_fee()
        ok, msg = await self.econ.withdraw(ctx, fee)
        if not ok:
            await ctx.reply(f"Cannot go on duty: {msg}", ephemeral=True); return
    ...
    async def off(self, ctx: commands.Context, unit_id: Optional[str] = None):
        ...
        units = await mid.units()
        if not units:
            await ctx.reply("You have no active units.", ephemeral=True); return
    ...
    async def status(self, ctx: commands.Context, to: str, unit_id: Optional[str] = None):
        try:
            ...
            await ctx.reply("Unknown status.", ephemeral=True); return
        mid = self.member(ctx.author)
        units = await mid.units()
        if not units:
            await ctx.reply("You have no units.", ephemeral=True); return
        if not unit_id or unit_id not in units:
    ...
    async def list(self, ctx: commands.Context):
        ...
        if not openi:
            await ctx.reply("No open incidents."); return

        def label(k): return {
            "engine":"Engine","ladder":"Ladder","rescue":"Rescue/USAR","ems":"EMS",
            "patrol":"Patrol/LEO","arff":"ARFF","tow":"Tow","command":"Command/BC"
        }[k]
    ...
    async def join(self, ctx: commands.Context, iid: str):
        ...
        if not p:
            p = Participant(user_id=ctx.author.id, unit_id=unit_id, unit_type=UnitType(unit["type"]), status=Status.ENROUTE)
        p.status = Status.ENROUTE; p.joined_ts = now_ts()
        inc.participants[key] = p
        await self._persist_inc(ctx.guild, inc)

        mid = self.member(ctx.author); units = await mid.units()
        units[unit_id]["status"] = Status.ENROUTE.value; units[unit_id]["joined_iid"] = iid
        await mid.units.set(units); await mid.active_unit.set(unit_id)
    ...
    async def arrive(self, ctx: commands.Context):
        ...
        if not iid:
            await ctx.reply("Your active unit is not assigned to an incident.", ephemeral=True); return
        gi = await self.guild(ctx.guild).incidents()
        data = gi.get(iid); inc = self._deserialize_inc(data)
        key = f"{ctx.author.id}:{unit_id}"
        p = inc.participants.get(key)
        if not p:
            await ctx.reply("This unit is not registered on that incident.", ephemeral=True); return

        first_before = any(pp.arrival_ts for pp in inc.participants.values())
        p.status = Status.ONSCENE; p.arrival_ts = now_ts()
        await self._persist_inc(ctx.guild, inc)
        ...
        # first-arrival stat
        if not first_before:
            stats = await mid.stats(); stats["first_arrivals"] = stats.get("first_arrivals", 0) + 1; await mid.stats.set(stats)
    ...
    async def transport(self, ctx: commands.Context):
        ...
        if p.unit_type != UnitType.EMS:
            await ctx.reply("Only EMS units can mark transport.", ephemeral=True); return
        p.status = Status.TRANSPORT; p.transport_ts = now_ts()
        await self._persist_inc(ctx.guild, inc)
    ...
    async def clear(self, ctx: commands.Context):
        ...
        if not p:
            await ctx.reply("This unit is not registered on that incident.", ephemeral=True); return
        p.status = Status.CLEARING; p.clear_ts = now_ts()
        await self._persist_inc(ctx.guild, inc)

        mid = self.member(ctx.author); units = await mid.units()
        units[unit_id]["status"] = Status.AVAILABLE.value; units[unit_id]["joined_iid"] = ""
        await mid.units.set(units)
