...
class MissionFormatter:
    ...
    def _format_prerequisites(mission_data: Dict) -> str:
        """Format unlock prerequisites with separate sections for buildings and trainings."""
        ...
        # Separate building requirements and training requirements
        building_reqs = []
        training_reqs = []
        ...
        for prereq_key, prereq_value in prerequisites.items():
            ...
            # Check if this is a training/education requirement
            if isinstance(prereq_value, dict) and any(k in TRAININGS for k in prereq_value.keys()):
                # This is a personnel education requirement
                for training_key, training_count in prereq_value.items():
                    training_name = TRAININGS.get(training_key, format_field_name(training_key))
                    training_reqs.append(f"- {training_count} {training_name}")
            elif '_extension' in prereq_key or '_count' in prereq_key or prereq_key in ['tow_trucks']:
        ...
        # Format training requirements
        if training_reqs:
            lines.append("")
            lines.append("**Required Personnel Training:**")
            lines.extend(training_reqs)
