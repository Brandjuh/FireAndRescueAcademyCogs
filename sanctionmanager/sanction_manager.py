...
class SummarySanctionView(discord.ui.View):
    ...
    async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        roles_removed = []
        if self.target_discord_user and isinstance(self.target_discord_user, discord.Member):
            role_ids_to_remove = []
            
            if self.sanction_type == "Kick":
                role_ids_to_remove = await self.cog.config.guild(guild).auto_remove_roles_on_kick()
            elif self.sanction_type == "Ban":
                role_ids_to_remove = await self.cog.config.guild(guild).auto_remove_roles_on_ban()
            
            if role_ids_to_remove:
                for role_id in role_ids_to_remove:
                    role = guild.get_role(role_id)
                    if role and role in self.target_discord_user.roles:
                        try:
                            await self.target_discord_user.remove_roles(role, reason=f"Auto-remove on {self.sanction_type}: Sanction #{sanction_id}")
                            roles_removed.append(role.name)
                            log.info(f"Removed role {role.name} from {self.target_discord_user} due to {self.sanction_type}")
                        except discord.Forbidden:
                            log.error(f"Failed to remove role {role.name} from {self.target_discord_user}: Missing permissions")
                        except Exception as e:
                            log.error(f"Failed to remove role {role.name} from {self.target_discord_user}: {e}")
        ...
        if roles_removed:
            log_embed.add_field(name="Roles Auto-Removed", value=", ".join(roles_removed), inline=False)
...
class SanctionsManager(commands.Cog):
    ...
    async def sanctionset(self, ctx: commands.Context):
        ...
        txt = (
            ...
            f"Auto-remove roles on Kick: {len(conf.get('auto_remove_roles_on_kick', []))} role(s)\n"
            f"Auto-remove roles on Ban: {len(conf.get('auto_remove_roles_on_ban', []))} role(s)\n"
        )
    ...
    @sanctionset.command(name="autoremoverole")
    @commands.admin()
    @commands.guild_only()
    async def autoremoverole(self, ctx: commands.Context, sanction_type: str, role: discord.Role):
        """Add a role to be automatically removed for a sanction type.
        
        Sanction types: kick, ban
        Example: [p]sanctionset autoremoverole kick @Verified
        """
        sanction_type = sanction_type.lower()
        
        if sanction_type not in ["kick", "ban"]:
            await ctx.send("❌ Invalid sanction type. Use: `kick` or `ban`")
            return
        
        config_key = f"auto_remove_roles_on_{sanction_type}"
        async with self.config.guild(ctx.guild).get_attr(config_key)() as roles:
            if role.id not in roles:
                roles.append(role.id)
                await ctx.send(f"✅ {role.mention} will now be automatically removed when someone is {sanction_type}ed.")
            else:
                await ctx.send(f"⚠️ {role.mention} is already configured for auto-removal on {sanction_type}.")

    @sanctionset.command(name="keeprole")
    @commands.admin()
    @commands.guild_only()
    async def keeprole(self, ctx: commands.Context, sanction_type: str, role: discord.Role):
        """Stop auto-removing a role for a sanction type.
        
        Sanction types: kick, ban
        Example: [p]sanctionset keeprole kick @Verified
        """
        sanction_type = sanction_type.lower()
        ...
        async with self.config.guild(ctx.guild).get_attr(config_key)() as roles:
            if role.id in roles:
                ...
                await ctx.send(f"✅ {role.mention} will no longer be auto-removed on {sanction_type}.")
            else:
                await ctx.send(f"⚠️ {role.mention} is not configured for auto-removal on {sanction_type}.")

    @sanctionset.command(name="autoremovedlist")
    @commands.admin()
    @commands.guild_only()
    async def autoremovedlist(self, ctx: commands.Context):
        """Show all roles that are automatically removed on kick/ban."""
        kick_role_ids = await self.config.guild(ctx.guild).auto_remove_roles_on_kick()
        ban_role_ids = await self.config.guild(ctx.guild).auto_remove_roles_on_ban()
        
        embed = discord.Embed(
            title="⚙️ Auto-Remove Role Configuration",
            color=discord.Color.blue(),
            timestamp=datetime.now(timezone.utc)
        )
        ...
        # Kick roles
        if kick_role_ids:
            kick_roles = [ctx.guild.get_role(rid) for rid in kick_role_ids]
            kick_roles = [r.mention for r in kick_roles if r is not None]
            if kick_roles:
                embed.add_field(
                    name="🦵 Kicked Members",
                    value="\n".join(kick_roles),
        ...
        # Ban roles
        if ban_role_ids:
            ban_roles = [ctx.guild.get_role(rid) for rid in ban_role_ids]
            ban_roles = [r.mention for r in ban_roles if r is not None]
            if ban_roles:
                embed.add_field(
                    name="🔨 Banned Members",
                    value="\n".join(ban_roles),
        ...
        embed.set_footer(text=f"Use {ctx.prefix}sanctionset autoremoverole to add roles")
